...
var PDFFindBar = (function PDFFindBarClosure() {
  function PDFFindBar(options) {
    ...
    this.findField.addEventListener('input', function() {
      delete PDFJS.multipleSearchParam;
...
var PDFFindController = (function PDFFindControllerClosure() {
  ...
  PDFFindController.prototype = {
    ...
    calcFindMatch: function PDFFindController_calcFindMatch(pageIndex) {
      ...
      var noAccents = true;
      if (noAccents) {
        pageContent = removeDiacritics(pageContent);
        ...
        for (var i = 0; i < numPages; i++) {
          ...
          if (!(i in this.pendingFindMatches)) {
            ...
            this.extractTextPromises[i].then(function(pageIdx) {
              ...
              if (PDFJS.multipleSearchParam === undefined) {
                self.calcFindMatch(pageIdx);
              } else {
                if(page === pageIdx) {
                  for (var j = 0; j < PDFJS.multipleSearchParam.length; j++) {
                    var tempTerm = PDFJS.multipleSearchParam[j].replace(/\=/ig, ' ');
                    tempTerm = tempTerm.trim();
                    self.state.query = tempTerm;
                    self.calcFindMatch(pageIdx);
                  }
                }
              }
...
var PDFURLFinder = (function PDFFindBarClosure() {
    function PDFURLFinder(page) {
        try {
            ...
            if (typeof searchParams.h == 'undefined') {
              return;
            }

            PDFJS.multipleSearchParam = decodeURIComponent(searchParams.h).split(',');
